...
- `intellij-plugin`: JetBrains IntelliJ Platform 插件源码，依赖 EmmyLua 主插件 (`com.tang`)。
...
当前原生调试工具仅支持 Windows。IntelliJ 版本复用 EmmyLua 主插件的调试协议实现；VS Code 版本负责启动、注入并输出连接端口，完整断点、变量、调用栈体验需要 VS Code 侧已有兼容 Emmy/Lua 调试适配器配合。
